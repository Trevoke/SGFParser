...
In most situations (one game, no variations), this is all you'll need:
...
When you save an SGF, it will also automatically indent it to make it a little easier for people to parse. There are a few ways to use it.

 require 'sgf'
 sgf = File.read('file.sgf')
 file = SGF::Parser.new.parse(sgf).save('destination.sgf')
 ----
