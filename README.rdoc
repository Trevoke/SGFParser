...
WARNING: An implementation requirement is to make sure any closing bracket (']') inside a comment is escaped: '\]'. If this is not done, you will be one sad panda! This library will do this for you upon saving, but will most likely die horribly on anything which does not follow this rule.
...
1) Error if first two characters are not (;
2) Warning if FF is not specified in first node
2) Warning if a multiple-entry identity only has one property